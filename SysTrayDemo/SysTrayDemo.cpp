...
#define sizeofstr(str)   (sizeof(str) / sizeof(str[0]))
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_USER_SHELLICON:
		...
		case WM_LBUTTONDOWN:
			//MessageBox(NULL, _T("Left-click"), _T("Click"), MB_OK);
			...
			lstrcpy(IconData.szInfo, L"Test balloon tip");
			lstrcpy(IconData.szInfoTitle, L"Test Title");
	...
	case WM_DEVICECHANGE:
	{
		...
		TCHAR strInfo[100] = { 0 };
		TCHAR strTitle[100] = L"Port Event";
		...
		case DBT_DEVICEARRIVAL:
			{
				//StringCchPrintf(strInfo, sizeofstr(strInfo),
				//	TEXT("Message: DBT_DEVICEARRIVAL\n"));
				PDEV_BROADCAST_HDR lpdb = (PDEV_BROADCAST_HDR)lParam;
				if (lpdb->dbch_devicetype == DBT_DEVTYP_PORT)
				{
					StringCchPrintf(strTitle, sizeofstr(strTitle),
						TEXT("Port Attached\n"));

					PDEV_BROADCAST_PORT lpdbp = (PDEV_BROADCAST_PORT)lpdb;
					StringCchPrintf(strInfo, sizeofstr(strInfo),
						TEXT("Port:  %s"), lpdbp->dbcp_name);
				}
			}
			break;
		...
		IconData.uCallbackMessage = WM_USER_SHELLICON;
		IconData.dwInfoFlags = NIIF_INFO;
		lstrcpy(IconData.szInfo, strInfo);
		lstrcpy(IconData.szInfoTitle, strTitle);
		IconData.uTimeout = 15000; // in milliseconds - this will be ignored in Win'10
